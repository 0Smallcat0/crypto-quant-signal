"""Goal P experiment 7: run the pre-registered 8-config Donchian family.

Executes every configuration in docs/research/GOALP_EXPERIMENT7_PREREGISTRATION.md
through the SAME registered-backtest pipeline as any other trial. BTC/ETH
universe (declared up front — the ladder engine aligns decision days by
intersection); regime-gate arms reuse the experiment-5 state machine.
"""
...
WINDOW_SETS = ((10, 20, 55, 110), (20, 55, 110, 220))
        ...
        for exit_mode in EXITS:
            for gated in GATES:
                ...
                result = run_registered_backtest(
                    ...
                    operator_note=(
                        "Goal P exp 7 family member: donchian ensemble "
                        f"windows={'+'.join(str(w) for w in windows)} exit={exit_mode} "
                        f"gate={'on' if gated else 'off'} "
                        "(GOALP_EXPERIMENT7_PREREGISTRATION.md)"
                    ),
                )
