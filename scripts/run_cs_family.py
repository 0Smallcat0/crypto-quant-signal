"""Goal P experiment 3: run the pre-registered 16-config cross-sectional family.

Executes every configuration in docs/research/GOALP_EXPERIMENT3_PREREGISTRATION.md
...
The 13-symbol universe and its quality gate live in
docs/research/UNIVERSE_EXPANSION.md; candles come from the pre-holdout
directory only. cs_decision_start pins every member's return series to the
registry window (2018-03-05 -> 2025-07-01) so the gate report's strict
alignment check can include the family.
"""
            ...
            for cadence in CADENCES:
                for absolute_filter in FILTERS:
                    ...
                    result = run_registered_backtest(
                        ...
                        operator_note=(
                            "Goal P exp 3 family member: cross-sectional momentum "
                            f"K={top_k} lookback={lookback} cadence={cadence} "
                            f"filter={'on' if absolute_filter else 'off'} "
                            "(GOALP_EXPERIMENT3_PREREGISTRATION.md)"
                        ),
                    )
                    ...
                    print(
                        f"trial {result.trial.trial_id}: K={top_k} lookback={lookback} "
