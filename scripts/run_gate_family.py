"""Goal P experiment 5: run the pre-registered 16-config regime-gate family.

Executes every configuration in docs/research/GOALP_EXPERIMENT5_PREREGISTRATION.md
through the SAME registered-backtest pipeline as any other trial. The cs
selection architecture is FIXED (K=2, 180d, monthly, absolute filter on);
only the directional-gate grid varies. No vol overlay (experiment 4 closed
that arm; the engine enforces mutual exclusion).
"""
            ...
            for hysteresis in HYSTERESES:
                for cadence in CADENCES:
                    parameters = BacktestParameters(
                        ...
                        cs_gate_sma_window=sma_window,
                        cs_gate_basis=basis,
                        cs_gate_hysteresis=Decimal(hysteresis),
                        cs_gate_cadence=cadence,
                    )
                    result = run_registered_backtest(
                        ...
                        operator_note=(
                            "Goal P exp 5 family member: cs K=2/180d/monthly/filter-on "
                            f"+ regime gate sma={sma_window} basis={basis} "
                            f"hysteresis={hysteresis} cadence={cadence} "
                            "(GOALP_EXPERIMENT5_PREREGISTRATION.md)"
                        ),
                    )
                    ...
                    print(
                        f"trial {result.trial.trial_id}: sma={sma_window} basis={basis} "
                        f"hysteresis={hysteresis} cadence={cadence} "
